...
export default function Header() {
  ...
  console.log(session);
  return (
    <div className="bg-white w-screen shadow-md sticky top-0 border py-3 px-6  h-[80px]">
            ...
            <>
              {session.user?.image ? (
                <Image
                  ...
                  className="h-8 max-md:hidden shadow-lg ring-1 rounded-full"
                  width={32}
                  height={32}
                />
              ) : (
                <div className="h-8 w-8 max-md:hidden bg-[url('../public/blank-profile-picture.webp')] bg-no-repeat bg-center bg-cover bg-gray-300  shadow-lg ring-black ring-1 rounded-full" />
              )}
  ...
  return (
    <Link href={href}>
      <div className="flex cursor-pointer items-center gap-x-1 rounded-md py-2 px-2  hover:bg-gray-100">
        <Image
          src={icon}
          className="h-8 w-8 text-gray-500 max-md:hidden fill-white outline-dotted"
          alt=""
          width={32}
          height={32}
        />
